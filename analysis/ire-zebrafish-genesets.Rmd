---
title: "Defining IRE Gene Sets - Zebrafish"
...
- Define IRE gene sets for zebrafish.
...
2bit file provided as `AH65657` in the package `AnnotationHub()`.
...
writeXStringSet(utr3Seq, here("data","ire",
                              "zebrafish","utr3.fa.gz"), compress = TRUE)
```
...
```{r importSireUtr3}
ireUtr3 <- here("data", "ire", "zebrafish","utr3_sires.gff.gz") %>% 
	import.gff() %>%
...
- As the `gene_name` field for *LSM2* contained spaces and the 
phrase *"(1 of many)"*, this needed to be manually corrected.
...
```{r importUtr5}
ireUtr5 <- here("data", "ire", "zebrafish","utr3_sires.gff.gz") %>% 
	import.gff() %>%
	as.data.frame(stringsAsFactors = FALSE) %>%
	as_tibble()
```
...
ireUtr5 %<>%
	...
	dplyr::select(-strand) %>%
	separate(seqnames, c("transcript_id", "gene_name", "location"), 
	         sep = "_") %>%
	mutate(location = str_replace(location, "chrUn-", "chrUn_")) %>%
	separate(
		location, 
	...
	makeGRangesFromDataFrame(
		...
		strand.field = "strand")
ireUtr5$gene_id <- mcols(gr)$gene_id[match(ireUtr5$transcript_id, 
                                           gr$transcript_id)]
```
```{r include=FALSE}
ireUtr5 <- readRDS(here("data", "ire", "zebrafish", "ireUtr5.rds"))
```
...
- There are 30 of these genes in total (see below) and overall it seems as 
if the 3' and 5' IREs may be associated with alternative splicing in some 
cases. 
...
```{r eval=FALSE}
ireUtr3 %>% saveRDS(here("data", "ire", "zebrafish", "ireUtr3.rds"))
ireUtr5 %>% saveRDS(here("data", "ire", "zebrafish", "ireUtr5.rds"))
```

- The 3' and 5' IRE gene sets

```{r eval=FALSE}
ireGenes %>% saveRDS(here("data", "ire", "zebrafish", "ireGenes.rds"))

ireGenes %>% lapply(function(x){
  x %>% 
    as.data.frame() %>%
    set_colnames("ensembl_gene_id")
}) %>% write.xlsx(here("data", "ire", "zebrafish", "ireGenes.xlsx"))
```

- The genes containing both 3' and 5' IREs

```{r eval=FALSE}
ire3and5 %>% saveRDS(here("data", "ire", "zebrafish", "ire3and5.rds"))
ire3and5 %>% write.csv(here("output", "zebrafish", "ire3and5.csv"))
```
