...
2bit file provided as `AH64629` in the package `AnnotationHub()`.
...
writeXStringSet(utr3Seq, here("data","ire",
                              "human","utr3.fa.gz"), compress = TRUE)
```
...
names(utr5Seq) <- paste(utr5$transcript_id, utr5$gene_name, granges(utr5),
                        sep = "_")
writeXStringSet(utr3Seq, here("data","ire",
                              "human","utr5.fa.gz"), compress = TRUE)
```
...
```{r importSireUtr3}
ireUtr3 <- here("data", "ire", "human","utr3_sires.gff") %>% 
	import.gff() %>%
...
```{r importUtr5}
ireUtr5 <- here("data", "ire", "human","utr5_sires.gff") %>% 
	import.gff() %>%
	as.data.frame(stringsAsFactors = FALSE) %>%
	as_tibble()
```
...
- In mouse and zebrafish we detected genes with both 3' and 5' IREs and 
these were removed due to a lack of a simple interpretation regarding 
their stability changes under iron dyshomeostasis. 

- Here we also check if there are also genes in human which have both 
3' and 5' IREs:
...
```{r eval=FALSE}
ireUtr3 %>% saveRDS(here("data", "ire", "human", "ireUtr3.rds"))
ireUtr5 %>% saveRDS(here("data", "ire", "human", "ireUtr5.rds"))
```

- The 3' and 5' IRE gene sets

```{r eval=FALSE}
ireGenes %>% saveRDS(here("data", "ire", "human", "ireGenes.rds"))

ireGenes %>% lapply(function(x){
  x %>% 
    as.data.frame() %>%
    set_colnames("ensembl_gene_id")
}) %>% write.xlsx(here("data", "ire", "human", "ireGenes.xlsx"))
```

- The genes containing both 3' and 5' IREs

```{r eval=FALSE}
ire3and5 %>% saveRDS(here("data", "ire", "human", "ire3and5.rds"))
ire3and5 %>% write.csv(here("data","ire", "human", "ire3and5.csv"))
```
