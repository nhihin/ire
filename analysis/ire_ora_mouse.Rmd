---
title: "Over-representation of IRE genes in MSigDB gene sets - Mouse"
author: "Nhi Hin"
date: "2020-04-22"
...
```{r}
mouseGenes <- as.data.frame(org.Mm.egENSEMBL)$ensembl_id
...
gs %<>% rowwise() %>% mutate(
  ...
  universe_with_ire = sum(mouseGenes %in% allIREGenes & 
                            !(mouseGenes %in% ids)), 
  universe_without_ire = (length(mouseGenes) - universe_with_ire),
  ...
  universe_with_ire3 = sum(mouseGenes %in% ireGenes$ire3_all & 
                             !(mouseGenes %in% ids)), 
  universe_without_ire3 = (length(mouseGenes) - universe_with_ire3),
  ...
  universe_with_ire5 = sum(mouseGenes %in% ireGenes$ire5_all & 
                             !(mouseGenes %in% ids)), 
  universe_without_ire5 = (length(mouseGenes) - universe_with_ire5)
) %>% 
  ungroup()
...
overlapPlot <- overlapDf %>% 
  ...
  ggplot(aes(x = str_to_title(stringr::str_wrap(geneset, 28)) %>% 
...
```{r eval=FALSE}
gs %>% saveRDS(here("data", "ire_ora", "mouse", "gs.rds"))
```

- MSigDB gene sets with significant over-representation of IRE gene sets

```{r eval=FALSE}
sigGs %>% saveRDS(here("data", "ire_ora", "mouse", "sigGs.rds"))
sigGs %>% write_csv(here("data", "ire_ora", "mouse", "sigGs.csv"))
```
...
```{r eval=FALSE}
overlapPlot %>% export::graph2pdf(here("data", "ire_ora", "mouse", 
                                       "overlapPlot"))
```
...
```{r eval=FALSE}
nodesDf2 %>% write_tsv(here("data", "ire_ora", "mouse", "nodes.tsv"))
edgeDf %>% write_tsv(here("data", "ire_ora", "mouse", "edges.tsv"))
```
